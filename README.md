...
- **原版全物品目錄** — 完整支援 Minecraft 字典物品，搭配生存獲取過濾器排除指令方塊與除錯方塊
- **13 大分類瀏覽** — 頂層獨立「礦物」分頁（細分「方塊類礦物（絲綢鎬採集）」與「無絲綢鎬採集礦物」）、方塊、工具、武器、護甲、材料等
- **管理員自訂物品上架系統** — 支援 GUI 拖放工作台與 `/shop add` 指令，完整保留自訂名稱、Lore、附魔與 NBT
...
1. 將 `ashop-1.8.0-beta.1.jar` 放入 `plugins/` 資料夾
