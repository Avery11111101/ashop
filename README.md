...
> ⚠️ **專案開發聲明**：本專案除了構想在 Minecraft 遊戲中進行實機測試之外，基本上全部程式碼與文檔均由 AI 生成與撰寫，人類開發者（Avery）僅進行架構審核以及功能發想與引導。
...
- **收購箱價值估計與全面統計** — 開啟收購箱（`/shop sell`）時，底部總計按鈕完整顯示放入物品總組數與個數、全部物品的商店購買總估值、可收購與不可收購統計、可收購原買價與現售金額對比以及預估回收折率；放入各商品亦即時標註商店原買價，讓出售價值一目了然
- **全商品統一分頁瀏覽** — 主選單（Slot 49，知識之書圖示）提供「全部商品」快捷頁面，一站式分頁查看全伺服器所有已啟用分類之商品，支援即時動態定價、趨勢漲跌、左鍵購買與管理員右鍵編輯
- **可視化分類與子分類即時建立** — 主畫面左下角（Slot 45，箱子圖示）可直接點擊新增頂層新分類；進入各分類或商品目錄頁面（Slot 46，儲物車圖示）可一鍵在當前目錄下建立全新子分類，支援互動對話框輸入 ID 與自訂名稱，無需重啟伺服器或進入後台建立 YAML 資料夾
- **管理員自訂物品上架系統** — 支援 GUI 拖放工作台（分類列表與上層分類 Slot 47）與 `/shop add` 指令，可放入任何原版或帶有自訂 NBT/名稱/Lore/附魔的自訂裝備販售，自動判定最深層葉子子分類，上架後自動跳轉至目標頁面
...
1. 將 `ashop-1.8.0-beta.9.jar` 放入 `plugins/` 資料夾
